...
void Continuous_knapsack(double vesrukzaka, int n) 
{
	...
	for (int i = n - 1; i >= 0; i--) 
	{
		if (cena[i][1] >= vesrukzaka) 
		{
			otvet[(int)cena[i][3]] = (cena[i][1] - vesrukzaka) / cena[i][1];
			vesrukzaka = 0;
			sum = sum + cena[i][2] * ((cena[i][1] - vesrukzaka) / cena[i][1]);
			break;
		}
		else 
		{
			otvet[(int)cena[i][3]] = 1;
			vesrukzaka = vesrukzaka - cena[i][1];
			sum = sum + cena[i][2];
		}
	}
	for (int i = 0; i < n; i++) 
